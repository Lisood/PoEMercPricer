...
if (-not $proc) {
    ...
    Get-Process poemercpricer, poemercpricer.exe -ErrorAction SilentlyContinue | Format-Table Id, ProcessName, MainWindowTitle | Out-String
    exit 1
}
...
$bmp.Save((Resolve-Path .).Path + "\" + $Out.Replace("/", "\"), [System.Drawing.Imaging.ImageFormat]::Png)
$g.Dispose()
$bmp.Dispose()
