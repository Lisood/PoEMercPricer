...
pub fn score(merc: &Mercenary) -> ScoreResult {
    ...
    let vaal_t3_edwa = vaal
        .map(|s| s.support_tier(&["edwa"]) as u8 == 3)
        .unwrap_or(false);
    let vaal_t3_hypothermia = vaal
        .map(|s| s.support_tier(&["hypothermia"]) as u8 == 3)
        .unwrap_or(false);
    ...
    let i_icicle = present(merc.has_skill(&["Icicle Rain"]));
    ...
    let premium_two_row =
        mirror.is_some() && t_vaal_return > 0.0 && t_ice_return > 0.0 && t_ice_gmp > 0.0;
    let jackpot =
        i_ice > 0.0 && i_vaal > 0.0 && i_icicle == 0.0 && (premium_vaal_row || premium_two_row);

    let mut highlights = Vec::new();
    if i_ice > 0.0 && i_vaal > 0.0 && mirror.is_some() && t_vaal_return > 0.0 && i_icicle == 0.0 {
        highlights
            .push("Market core: Vaal Ice Shot + Mirror Arrow + Return on Vaal Ice Shot".into());
    }
    if jackpot {
        highlights.push(
            "Market-proven premium: Vaal Return plus either Vaal T3 EDWA+Hypothermia or Ice Return+GMP"
                .into(),
        );
    }
    if i_icicle > 0.0 {
