...
impl Mercenary {
    ...
    pub fn t(&self, skill: &str, supports: &[&str]) -> f32 {
        self.skill(&[skill]).map(|s| s.t(supports)).unwrap_or(0.0)
    }

    pub fn i(&self, names: &[&str]) -> i32 {
        i32::from(self.has_skill(names))
    }
}
...
pub fn interpret_score(score: f32, family: &str) -> (&'static str, &'static str) {
    let check_at = if family == "combatant" { 65.0 } else { 55.0 };
    if score < 50.0 {
