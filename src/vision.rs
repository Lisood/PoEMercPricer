...
//! a roman-numeral tier I/II/III in the corner; that's the gem "level".
...
use crate::catalog::{
    compatible_supports_for_skill_tier, support_display_for_tier, try_parse_tier,
};
...
fn support_templates() -> &'static [SupportTemplate] {
    ...
    T.get_or_init(|| {
        ...
        for (name, bytes) in crate::icons::SUPPORTS {
            ...
            if let Ok(img) = image::load_from_memory(bytes) {
                let fine = resize_fine(&img.into_rgba8());
...
fn resize_fine(img: &RgbaImage) -> RgbaImage {
    resize_icon(img, 32, 32)
}

/// Area-weighted (Triangle) resampling for both templates and captured cells.
/// Nearest-neighbour dropped a third of the client's 48 px pixels, and that
/// aliasing held correct matches at 0.55-0.70: the Swift Affliction and
/// Infused Channelling cells scored below the floor and were reported as
/// unidentified "ring-and-hook" gems. Triangle lifts the mean exact-match
/// score from 0.70 to 0.85 on the fixture set (see docs/performance.md).
fn resize_icon(img: &RgbaImage, w: u32, h: u32) -> RgbaImage {
...
struct IconMatch {
    ...
    name: String,
    confidence: f32,
    /// Catalog identities still possible for an `ambiguous` result whose art
    /// matched decisively (byte-identical arts). Empty for exact matches and
    /// for below-floor fallbacks, which are too weak to resolve by elimination.
    candidates: Candidates,
}

/// A support gem cannot be socketed twice in one skill row. Use that rule to
/// resolve shared-art ambiguity by elimination: drop identities already
/// claimed exactly elsewhere in the row, and when N cells share the same
/// N-candidate set, all N identities must be present.
fn resolve_row_uniqueness(matches: &mut [Option<IconMatch>]) {
    loop {
        let exact: Vec<String> = matches
            .iter()
            .flatten()
            .filter(|m| m.canonical != "ambiguous")
            .map(|m| m.canonical.clone())
            .collect();
        let mut changed = false;
        for m in matches.iter_mut().flatten() {
            if m.canonical != "ambiguous" || m.candidates.is_empty() {
                continue;
            }
            let before = m.candidates.len();
            m.candidates
                .retain(|(canonical, _)| !exact.iter().any(|e| e.eq_ignore_ascii_case(canonical)));
            if m.candidates.len() != before {
                changed = true;
                m.name = m
                    .candidates
                    .iter()
            ...
            if m.candidates.len() == 1 {
                m.canonical = m.candidates[0].0.to_owned();
                m.name = m.candidates[0].1.to_owned();
                m.candidates.clear();
                changed = true;
            }
        }
        ...
        // Set consumption: N cells sharing one N-candidate set.
        let mut assigned = false;
        for index in 0..matches.len() {
            let Some(set) = matches[index]
                .as_ref()
                .filter(|m| m.canonical == "ambiguous" && !m.candidates.is_empty())
                .map(|m| m.candidates.clone())
            else {
                continue;
            };
            let members: Vec<usize> = matches
                .iter()
                .enumerate()
                .filter(|(_, other)| {
                    other
                        .as_ref()
                        .is_some_and(|o| o.canonical == "ambiguous" && o.candidates == set)
                })
                .map(|(i, _)| i)
                .collect();
            ...
            for (slot, cell) in members.into_iter().enumerate() {
                let m = matches[cell].as_mut().expect("member exists");
                m.canonical = set[slot].0.to_owned();
                m.name = set[slot].1.to_owned();
                m.candidates.clear();
            }
            assigned = true;
            break;
...
type Candidates = Vec<(&'static str, &'static str)>;

/// Compatible catalog identities per template, computed once per skill row
/// rather than per template per cell.
fn row_candidates(skill_name: &str, tier: u8) -> Vec<Candidates> {
    support_templates()
        .iter()
        .map(|template| compatible_candidates(template, skill_name, tier))
        .collect()
}

fn compatible_candidates(template: &SupportTemplate, skill_name: &str, tier: u8) -> Candidates {
...
/// `candidates[i]` is the compatible identity set of `support_templates()[i]`
/// for this skill row and tier (see [`row_candidates`]).
fn match_support_icon(
    ...
    let coarse_variants: Vec<_> = variants
        .iter()
        .map(|variant| resize_icon(variant, 16, 16))
        .collect();
    let templates = support_templates();
    let mut shortlist: Vec<(usize, f32)> = templates
        .iter()
        .enumerate()
        .filter(|(index, _)| !candidates[*index].is_empty())
        .map(|(index, template)| {
    ...
    if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
        let (runner_index, runner) = ranked.get(1).copied().unwrap_or((index, 0.0));
        let mut unconstrained: Vec<(f32, &str)> = templates
            .iter()
            .map(|template| {
                let score = (-2..=2)
                    .flat_map(|dy| (-2..=2).map(move |dx| (dx, dy)))
                    ...
                    .max_by(f32::total_cmp)
                    .unwrap_or(0.0);
                (score, template.art_keys[0].as_str())
            })
            .collect();
        unconstrained.sort_by(|a, b| b.0.total_cmp(&a.0));
        eprintln!(
            "  any-art {skill_name} @{},{}: {:?}",
            cell.0,
            cell.1,
        ...
        eprintln!(
            "  icon {skill_name} @{},{} side={}: best={score:.3} runner={runner:.3} candidates={:?} runner_candidates={:?}",
            cell.0, cell.1, cell.2, candidates[index], candidates[runner_index]
        );
    }
    ...
    // Below the floor, retain the three best catalog-backed candidate groups
    // instead of inventing one identity.
    //
    // The generic silver/gold "Merc*SupportGem" arts are low-detail attractors:
    // they score 0.52-0.61 against gems whose live art is missing from the
    // template set (two ring-and-hook gems so far), while true placeholder
    // cells score 0.77 or better across every fixture. Real arts have never
    // matched correctly below 0.552 (Vortex, Swift Affliction) and were wrong
    // at 0.521 (Frost Bomb). These constants are fixture-derived; the upgrade
    // path is adding the missing live arts as templates.
    let placeholder = templates[index]
        .art_keys
        .iter()
        .all(|art_key| art_key.starts_with("merc"));
    let floor = if placeholder { 0.70 } else { 0.54 };
    if score < floor {
        let mut low_confidence: Candidates = Vec::new();
    ...
    let mut candidates_for_best = candidates[index].clone();
    ...
    if let Some((runner_index, runner_score)) = ranked.get(1).copied() {
        ...
        if score - runner_score < 0.001 {
            for &candidate in &candidates[runner_index] {
                if !candidates_for_best
                    .iter()
                    .any(|(canonical, _)| canonical.eq_ignore_ascii_case(candidate.0))
                {
                    candidates_for_best.push(candidate);
                }
            }
            candidates_for_best.sort_unstable_by(|a, b| a.0.cmp(b.0));
        }
    }
    if candidates_for_best.len() == 1 {
        return Some(IconMatch {
            canonical: candidates_for_best[0].0.to_owned(),
            name: candidates_for_best[0].1.to_owned(),
            confidence: score.clamp(0.0, 1.0),
            candidates: Vec::new(),
        });
    }
    ...
    Some(IconMatch {
        ...
        name,
        confidence: score.clamp(0.0, 1.0),
        candidates: candidates_for_best,
    })
}
...
/// square 10 to 15% too tight or too loose.
fn normalized_cell_variants(img: &RgbaImage, cell: (u32, u32, u32, u32)) -> Vec<RgbaImage> {
    ...
    [85_u32, 100, 115]
        ...
        .map(|percent| {
            ...
            resize_fine(&crop(img, left, top, scaled_w, scaled_h))
        })
        .collect()
}
...
fn icon_similarity(a: &RgbaImage, b: &RgbaImage, dx: i32, dy: i32) -> f32 {
    ...
    // similarly shaped but differently coloured arts from tying it. Gradient
    // NCC, brightness-gain and chromaticity colour terms were each measured
    // on the fixture set and lost exact cells or cost time; see
    // docs/performance.md.
    let (ncc, mae) = masked_similarity_shifted(a, b, dx, dy);
    0.65 * ncc + 0.35 * mae
}

fn masked_ncc_shifted(a: &RgbaImage, b: &RgbaImage, dx: i32, dy: i32) -> f32 {
    masked_similarity_shifted(a, b, dx, dy).0
}

/// Returns `(normalized cross-correlation, 1 - mean absolute error)` of the
/// unmasked template pixels in one pass.
fn masked_similarity_shifted(a: &RgbaImage, b: &RgbaImage, dx: i32, dy: i32) -> (f32, f32) {
    ...
    let mae = 1.0 - error as f32 / (count as f32 * 3.0 * 255.0);
    let n = count as f64;
    let mut num = 0.0f64;
    let mut da = 0.0f64;
    let mut db = 0.0f64;
    ...
    let den = (da * db).sqrt();
    let ncc = if den < 1e-3 { 0.0 } else { (num / den) as f32 };
    (ncc, mae)
}

/// Full-screen game captures are landscape and at least 1024 wide; cropped
/// panel screenshots and clipboard images are portrait or near-square. The
/// aspect floor is 1.2 so that 4:3 (1024x768, 1152x864) and 5:4 (1280x1024)
/// monitors are still treated as full screens.
pub fn is_fullscreen_capture(width: u32, height: u32) -> bool {
    width >= 1024 && width * 5 >= height * 6
}
...
pub fn guess_panel(img: &RgbaImage, lines: &[OcrLine]) -> PanelBox {
    ...
    if !is_fullscreen_capture(iw, ih) {
        return PanelBox {
            x: 0,
            y: 0,
            w: iw,
    ...
    if xs.len() >= 4 {
        ...
        // Extending x1 to the nominal panel edge ((iw + w) / 2) was tried: it
        // shifts the support column start on the Danalla fixture and admits
        // gold HUD cells on the 1440p fixtures, so the OCR-derived edge stays.
...
#[cfg(test)]
fn find_support_cells(img: &RgbaImage, row_y: u32, row_h: u32) -> Vec<(u32, u32, u32, u32)> {
...
fn find_support_cells_between(
    ...
    // Smaller than one support cell: nothing to find, and the clamps below
    // would panic on inverted bounds.
    if y1 <= y0 || x_max <= x_min || iw < 24 || ih < 24 {
        return Vec::new();
    }
    let mw = x_max - x_min;
    ...
    let side = (row_h * 6 / 7).clamp(24, 128).min(iw).min(ih);
    ...
    if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
        eprintln!("  side={side} clusters={clusters:?}");
    }
    let shaped: Vec<_> = clusters
        .into_iter()
        ...
        .collect();
    let clusters = snap_clusters_to_grid(shaped, side);
    ...
    let centers: Vec<u32> = clusters
        ...
        .collect();
    // Letting the observed pitch grow the cell beyond `side` (for synthetic
    // 150% DPI fixtures) and measuring the gold frame height directly were
    // both tried: each fixed the scaled cells but moved or mis-tiered cells
    // on the real 100% captures, so the cap stays.
...
/// Keep the gold clusters that sit on the regular support-cell grid.
///
/// Only clusters with plenty of gold (`side * 2`) anchor the grid. Icons
/// whose art is mostly gem colour (the Return boomerang, for example) leave
/// far less gold than the threshold, so a weaker cluster is kept when it lands
/// on a grid slot, and a single empty slot between two occupied ones is filled:
/// the in-game grid is left-aligned and contiguous, so such a hole is always a
/// real cell. Wider holes are not filled so one stray gold blob cannot invent a
/// run of cells.
fn snap_clusters_to_grid(shaped: Vec<(u32, u32, u32)>, side: u32) -> Vec<(u32, u32, u32)> {
    let center = |cluster: &(u32, u32, u32)| cluster.0 + (cluster.1 - cluster.0) / 2;
    let strong: Vec<u32> = shaped
        .iter()
        .filter(|(_, _, area)| *area >= side * 2)
        .map(center)
        .collect();
    let Some(&first) = strong.first() else {
        return Vec::new();
    };
    let mut pitches: Vec<u32> = strong
        .windows(2)
    ...
    let pitch = pitches
        ...
        .copied()
        .unwrap_or(side * 6 / 5);
    let tolerance = pitch / 3;
    let nearest = |expected: u32| {
        shaped
            .iter()
            .filter(|cluster| center(cluster).abs_diff(expected) <= tolerance)
            .min_by_key(|cluster| center(cluster).abs_diff(expected))
            .copied()
    };
    let strong_at = |expected: u32| {
        strong
            .iter()
            .any(|center| center.abs_diff(expected) <= tolerance)
    };
    let mut cells = Vec::new();
    let mut expected = first;
    loop {
        match nearest(expected) {
            Some(cluster) => cells.push(cluster),
            // Only a strong cell on the far side of a hole proves the grid
            // continues; a weak blob there is panel decoration, not a gem.
            None if strong_at(expected + pitch) => {
                cells.push((expected.saturating_sub(side / 2), expected + side / 2, 0));
            }
            None => break,
        }
        expected += pitch;
...
pub fn read_cell_tier(img: &RgbaImage, cell: (u32, u32, u32, u32)) -> SupportTier {
    cell_tier(img, cell).0
}

/// The tier and whether it was guessed (T2 default) rather than read.
fn cell_tier(img: &RgbaImage, cell: (u32, u32, u32, u32)) -> (SupportTier, bool) {
    let (x, y, w, h) = cell;
    let x0 = x + w * 5 / 12;
    let y0 = y + h * 13 / 24;
    ...
    if x1 <= x0 || y1 <= y0 {
        return (SupportTier::T2, true);
    }
    // Stroke-count on bright roman numerals: I=1, II=2, III=3.
    match count_roman_strokes(img, x0, y0, x1, y1) {
        1 => (SupportTier::T1, false),
        2 => (SupportTier::T2, false),
        3 => (SupportTier::T3, false),
        _ => ocr_crop(img, x0, y0, x1 - x0, y1 - y0)
            .ok()
            .and_then(|lines| try_parse_tier(&lines.join(" ")))
            .map_or((SupportTier::T2, true), |tier| {
                (SupportTier::from_u8(tier), false)
            }),
    }
}
...
pub fn supports_for_skill_row(
    ...
    let row_h = skill_line.h.max(28).saturating_mul(2).min(150);
    let cy = skill_line.y + skill_line.h / 2;
    let y0 = cy.saturating_sub(row_h / 2);
    if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
        eprintln!("  find cells y={y0} h={row_h} line={:?}", skill_line.text);
    }
    // The panel box's left edge comes from OCR header lines. A hover tooltip
    // that clips the header (for example the Wager tooltip hiding `Infamous`)
    // shifts that edge right and the 48% start then cuts the first icon
    // column. Icons always sit right of the skill text, so accept whichever
    // anchor starts earlier.
    let support_x0 = panel
        .x
        .saturating_add(panel.w * 48 / 100)
        .min(skill_line.x + skill_line.w + 4);
    ...
    if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
        eprintln!(
            "  panel x={} w={} support_x=[{support_x0}, {support_x1}) line x={} w={}",
            panel.x, panel.w, skill_line.x, skill_line.w
        );
        eprintln!(
            "  cells y={} h={}: {} in {:.1} ms",
            y0,
    ...
    // Compatibility pools are only needed for tiers that actually appear in
    // the row; rows without cells (auras) skip the catalog pass entirely.
    let candidates_by_tier: [std::cell::OnceCell<Vec<Candidates>>; 3] = Default::default();
    let tiers: Vec<(SupportTier, bool)> = cells.iter().map(|cell| cell_tier(img, *cell)).collect();
    let mut matches: Vec<Option<IconMatch>> = cells
        .iter()
        .zip(&tiers)
        .map(|(cell, (tier, _))| {
            match_support_icon(
                ...
                *cell,
                skill_name,
                candidates_by_tier[*tier as usize - 1]
                    .get_or_init(|| row_candidates(skill_name, *tier as u8)),
            )
        })
        .collect();
    let before: Vec<String> = matches
        .iter()
        .map(|m| m.as_ref().map(|m| m.canonical.clone()).unwrap_or_default())
        .collect();
    resolve_row_uniqueness(&mut matches);
    cells
        .into_iter()
        .zip(tiers)
        .zip(matches)
        .zip(before)
        .map(|(((cell, (tier, guessed)), matched), before)| {
            let resolved_by_row = matched
                .as_ref()
                .is_some_and(|m| before == "ambiguous" && m.canonical != "ambiguous");
            ...
            let name = if !matches!(canonical.as_str(), "unknown" | "ambiguous") {
                ...
                matched
                    ...
                    // The tier is carried on the gem, and every display path
                    // appends it, so naming it here reads as "... T2 T2".
                    .unwrap_or_else(|| "Unresolved support".into())
            };
            let mut confidence = matched
                .as_ref()
                .map(|matched| matched.confidence)
                .unwrap_or(0.4);
            let mut raw = format!("icon@{},{} T{}", cell.0, cell.1, tier as u8);
            if guessed {
                confidence = confidence.min(0.5);
                raw.push_str("; tier guessed");
            }
            if resolved_by_row {
                raw.push_str("; shared art resolved by row uniqueness");
            }
            if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
                eprintln!(
                    "  result {skill_name} @{},{} T{} {canonical} conf={confidence:.3} guessed={guessed} name={name:?}",
                    cell.0, cell.1, tier as u8
                );
            }
            SupportGem {
                name,
...
fn is_gold_frame(r: u8, g: u8, b: u8) -> bool {
    ...
    (18.0..=58.0).contains(&h) && s >= 0.18 && v >= 0.28
}
...
mod tests {
    #[test]
    #[cfg(windows)]
    fn first_support_column_survives_a_right_shifted_panel_box() {
        // Lazeth capture: Despair's two icons start at x=1282. A Wager hover
        // tooltip clipped the header line, shifting the panel box right, so a
        // 48%-of-width start (1304) would cut the first icon.
        let img = image::open("samples/cruel_mistress_lazeth.png")
            .expect("fixture")
            .to_rgba8();
        let line = crate::winocr::OcrLine {
            text: "DESPAIR".into(),
            x: 991,
            y: 807,
            w: 66,
        ...
        let shifted = super::PanelBox {
            x: 1010,
            y: 300,
            w: 714,
            h: 1000,
        };
        let supports = super::supports_for_skill_row(&img, &line, "Despair", shifted);
        assert_eq!(supports.len(), 2, "{supports:?}");
    }

    use super::*;

    #[test]
    fn fullscreen_predicate_uses_aspect_ratio_and_size() {
        assert!(is_fullscreen_capture(1366, 768));
        assert!(is_fullscreen_capture(2560, 1440));
        // 4:3 and 5:4 monitors are full screens too.
        assert!(is_fullscreen_capture(1024, 768));
        assert!(is_fullscreen_capture(1152, 864));
        assert!(is_fullscreen_capture(1280, 1024));
        assert!(is_fullscreen_capture(1154, 783));
        // Cropped panel screenshots are portrait, or tiny support strips.
        assert!(!is_fullscreen_capture(850, 1189));
        assert!(!is_fullscreen_capture(954, 1189));
        assert!(!is_fullscreen_capture(170, 80));
    }

    fn ambiguous(candidates: &[(&'static str, &'static str)]) -> Option<IconMatch> {
        Some(IconMatch {
            canonical: "ambiguous".into(),
            name: candidates
                .iter()
    ...
    fn exact(canonical: &str) -> Option<IconMatch> {
        Some(IconMatch {
    ...
    #[test]
    fn row_uniqueness_resolves_shared_art_by_elimination_and_pair_consumption() {
        // Orvan's Divine Retribution: cell 3 shares art between Cooldown
        // Recovery and Shock Chance, cell 4 is Cooldown Recovery exactly.
        let mut row = vec![
            exact("crit_chance"),
            ambiguous(&[
        ...
        resolve_row_uniqueness(&mut row);
        assert_eq!(row[1].as_ref().unwrap().canonical, "shock_chance");
        assert_eq!(row[1].as_ref().unwrap().name, "Shock Chance");

        // Sid's Spectral Throw: two cells share one two-identity set, so both
        // identities are present; three such cells cannot be resolved.
        let pair = [
            ("throwing_speed", "Throwing Speed"),
            ("trigger_radius", "Trigger Radius"),
        ];
        let mut row = vec![ambiguous(&pair), exact("gmp"), ambiguous(&pair)];
        resolve_row_uniqueness(&mut row);
        let names: Vec<_> = row.iter().flatten().map(|m| m.canonical.as_str()).collect();
        assert_eq!(names, ["throwing_speed", "gmp", "trigger_radius"]);

        let mut row = vec![ambiguous(&pair), ambiguous(&pair), ambiguous(&pair)];
        resolve_row_uniqueness(&mut row);
        assert!(row.iter().flatten().all(|m| m.canonical == "ambiguous"));

        // Below-floor fallbacks carry no candidate set and are never touched.
        let mut row = vec![
            exact("cdr"),
            Some(IconMatch {
                canonical: "ambiguous".into(),
                name: "Cooldown Recovery / Fork".into(),
                confidence: 0.4,
                candidates: Vec::new(),
            }),
        ];
        resolve_row_uniqueness(&mut row);
        assert_eq!(row[1].as_ref().unwrap().name, "Cooldown Recovery / Fork");
    }

    #[test]
    fn tiny_image_has_no_support_cells_and_does_not_panic() {
        let img = RgbaImage::new(10, 10);
        assert!(find_support_cells(&img, 0, 10).is_empty());
    }

    #[test]
    fn unreadable_tier_text_is_reported_as_guessed() {
        assert_eq!(try_parse_tier("gem"), None);
        assert_eq!(try_parse_tier("III"), Some(3));
        assert_eq!(try_parse_tier("t1"), Some(1));
    }
    ...
    fn alara_support_grid_names_and_tiers_are_read_from_pixels() {
            ...
            let matches: Vec<_> = cells
                ...
                .map(|cell| {
                    match_support_icon(
                        ...
                        *cell,
                        skill_names[index],
                        &row_candidates(skill_names[index], read_cell_tier(&img, *cell) as u8),
                    )
        ...
        let matches: Vec<_> = ice
            ...
            .map(|cell| {
                match_support_icon(
                    ...
                    *cell,
                    "Ice Shot",
                    &row_candidates("Ice Shot", read_cell_tier(&img, *cell) as u8),
                )
                .unwrap()
            })
            .collect();
        ...
        let second_wind = match_support_icon(
            ...
            mirror[2],
            "Mirror Arrow",
            &row_candidates("Mirror Arrow", read_cell_tier(&img, mirror[2]) as u8),
        )
        .unwrap();
        let second_cell = mirror[2];
        let sized = resize_fine(&crop(
            &img,
            second_cell.0,
            second_cell.1,
            second_cell.2,
        ...
        let ambiguous = match_support_icon(
            ...
            mirror[3],
            "Mirror Arrow",
            &row_candidates("Mirror Arrow", read_cell_tier(&img, mirror[3]) as u8),
        )
        .unwrap();
            ...
            .map(|cell| {
                match_support_icon(
                    ...
                    *cell,
                    ...
                    &row_candidates("Wave of Conviction of Trarthus", 3),
                )
            ...
            bridged
                ...
                .map(|cell| {
                    match_support_icon(
                        ...
                        *cell,
                        "Boiling Blood",
                        &row_candidates("Boiling Blood", read_cell_tier(&img, *cell) as u8),
                    )
                    .expect("every recovered support cell must remain identifiable")
                    .canonical
                })
                .collect::<Vec<_>>(),
            ["brutality", "swift_affliction", "more_duration"]
        );
    }
}
