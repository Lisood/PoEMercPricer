...
use crate::models::{infer_mercenary_skill_level, Mercenary, Skill, SupportGem};
use crate::parse::parse_ocr_lines;
use crate::vision::{crop, guess_panel, is_fullscreen_capture, supports_for_skill_row};
...
pub fn scan_rgba(img: &RgbaImage) -> Result<Mercenary> {
    ...
    if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
        eprintln!("OCR: {:.1} ms", started.elapsed().as_secs_f64() * 1_000.0);
    }
    scan_ocr_lines(img, raw_lines, offset_x, offset_y)
}

/// Everything after OCR; pure so it can be tested without Windows OCR.
fn scan_ocr_lines(
    img: &RgbaImage,
    raw_lines: Vec<OcrLine>,
    offset_x: u32,
    offset_y: u32,
) -> Result<Mercenary> {
    let started = std::time::Instant::now();
    ...
    // Each skill row's icon vision only reads the shared capture, so the six
    // rows run on scoped threads and join before anything is written back.
    let row = |skill: &Skill| -> Option<(String, Option<u32>, Vec<SupportGem>)> {
        let skill_started = std::time::Instant::now();
        let line = best_line_for_skill(&cleaned, skill)?;
        let supports = supports_for_skill_row(img, line, &skill.canonical, panel);
        if std::env::var_os("POEMERC_PROFILE_OCR").is_some() {
            eprintln!(
                "vision {}: {:.1} ms",
                skill.canonical,
        ...
        Some((
            line.text.clone(),
            parse_nearby_level(&cleaned, line).or(inferred_skill_level),
            supports,
        ))
    };
    let rows: Vec<_> = std::thread::scope(|scope| {
        let handles: Vec<_> = merc
            .skills
            .iter()
            .map(|skill| scope.spawn(|| row(skill)))
            .collect();
        handles
            .into_iter()
            .map(|handle| handle.join().expect("row vision thread panicked"))
            .collect()
    });
    for (skill, found) in merc.skills.iter_mut().zip(rows) {
        if let Some((raw, level, supports)) = found {
            skill.raw = raw;
            skill.level = level;
...
fn apply_visible_support_tooltip_hints(merc: &mut Mercenary, ocr_texts: &[String]) {
    // Canonical ids of ambiguous candidates whose exact title is visible.
    ...
    for (skill_index, skill) in merc.skills.iter().enumerate() {
        for (support_index, support) in skill.supports.iter().enumerate() {
            ...
            let matching_hints: Vec<_> =
                hints
                    .iter()
                    .filter(|hint| {
                        support.name.split(" / ").map(str::trim).any(|candidate| {
                            canonical_support(candidate).0.eq_ignore_ascii_case(hint)
                        })
                    })
                    .collect();
            ...
            let eligible_count = merc
                ...
                .count();
            if eligible_count == 1 {
                resolutions.push((skill_index, support_index, hint.clone()));
            }
        }
    }
...
fn skill_text_region(img: &RgbaImage) -> (RgbaImage, u32, u32) {
    let width = img.width();
    if !is_fullscreen_capture(width, img.height()) {
...
/// The skill level is printed on its own line beside the skill name, so take
/// the closest level text on the same row.
fn parse_nearby_level(lines: &[OcrLine], skill_line: &OcrLine) -> Option<u32> {
    static LEVEL: LazyLock<Regex> =
        LazyLock::new(|| Regex::new(r"(?i)\b(?:lv|lvl|level)\s*([1-9]|[1-3][0-9]|40)\b").unwrap());
    let centre = |line: &OcrLine| line.y + line.h / 2;
    let reach = skill_line.h.max(16) * 2;
    lines
        .iter()
        .filter(|line| centre(line).abs_diff(centre(skill_line)) <= reach)
        .filter_map(|line| LEVEL.captures(&line.text).map(|c| (line, c)))
        .min_by_key(|(line, _)| centre(line).abs_diff(centre(skill_line)))
        .and_then(|(_, c)| c[1].parse().ok())
}

pub fn cleanup_ocr(text: &str) -> String {
    static LVL: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\b(lvi|lv1)\b").unwrap());
    static Z_PREFIX: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)^z:\s*").unwrap());
    let t = text
        .replace('`', "'")
        .replace("lnfamous", "Infamous")
        .replace("Lnfamous", "Infamous");
    let t = LVL.replace_all(&t, "Lvl");
    Z_PREFIX.replace(&t, "").trim().to_string()
}
...
mod tests {
    ...
    #[test]
    fn nearby_level_reads_the_closest_level_line_on_the_row() {
        let line = |text: &str, y: u32| OcrLine {
            text: text.into(),
            x: 900,
            y,
            w: 100,
        ...
        let lines = [
            line("Lvl 83", 200),
            line("VIPER STRIKE", 814),
            line("Lvl 27", 820),
            line("Lvl 12", 880),
        ];
        assert_eq!(parse_nearby_level(&lines, &lines[1]), Some(27));
        assert_eq!(parse_nearby_level(&lines, &lines[0]), None);
    }

    #[test]
    fn zero_ocr_lines_is_an_error() {
        let error = scan_ocr_lines(&RgbaImage::new(64, 64), Vec::new(), 0, 0)
            .expect_err("empty OCR output must not produce an empty mercenary");
        assert!(error.to_string().contains("Warrant panel"));
    }

    #[test]
    fn cleanup_ocr_normalizes_lvl_case_insensitively() {
        assert_eq!(cleanup_ocr("Z:lnfamous Manyshot"), "Infamous Manyshot");
        assert_eq!(cleanup_ocr("LVI 83"), "Lvl 83");
        assert_eq!(cleanup_ocr("lv1 26"), "Lvl 26");
        assert_eq!(cleanup_ocr("Silvia"), "Silvia");
    }
    ...
    fn reconstructs_every_multiword_catalog_skill_at_every_word_boundary_and_row() {
        ...
        assert!(cases >= 5_900, "only exercised {cases} wrapped-name cases");
    }
