...
//! have a real 3.29 catalog identity and tier.  Finding all 16 cells on this
//! fixture while silently returning eight of them as `unknown` is a failure.
...
mod windows {
    ...
    fn alara_has_no_missing_or_placeholder_support_gems() {
        ...
        assert_eq!(
            merc.family, "manyshot",
            "class={} name={}",
            merc.class_name, merc.name
        );
        ...
        assert_eq!(
            merc.skills
                .iter()
                .map(|skill| (skill.canonical.as_str(), skill.level, skill.supports.len()))
                .collect::<Vec<_>>(),
            [
                ("Ice Shot", Some(26), 4),
                ("Vaal Grace", Some(26), 0),
                ("Icicle Rain", Some(26), 5),
                ("Mirror Arrow", Some(26), 4),
                ("Grace", Some(26), 0),
                ("Blink Arrow", Some(26), 3),
            ],
        ...
        // These identities look different enough that the scanner should
        // never fall back to reporting them as ambiguous.
        assert_row(
            &merc,
            "Ice Shot",
            &[
    ...
    fn alara_scan_stays_within_release_latency_budget() {
        ...
        eprintln!("warm Alara scan: {elapsed:?} (budget {budget:?})");
    }
